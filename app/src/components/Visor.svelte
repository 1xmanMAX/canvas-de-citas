<script>
  ...
  const srcdoc = $derived(a?.tipo === 'html' && texto
    ? (/<head[^>]*>/i.test(texto) ? texto.replace(/<head([^>]*)>/i, '<head$1><base target="_blank">') : '<base target="_blank">' + texto)
    : '')
  ...
  <section class="visor" class:grande={V.grande} aria-label="Visor de documento">
    ...
    <div class="v-cuerpo">
      ...
      {:else}
        ...
        <article class="lectura" class:plano={a.tipo === 'texto'} onmouseup={seleccionLocal} ontouchend={seleccionLocal}>
